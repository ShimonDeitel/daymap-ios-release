...
extension Color {
    static let dmAccent = Color(hex: "#007AFF")          // the single accent
    static let dmCard = Color(uiColor: .secondarySystemBackground)
    static let dmCard2 = Color(uiColor: .tertiarySystemBackground)
    static let dmField = Color(uiColor: .tertiarySystemFill)
    static let dmHair = Color(uiColor: .separator)
}

// MARK: - Flat surfaces (cards / pills / buttons)

extension View {
    func dmCard(cornerRadius: CGFloat = 20) -> some View {
        self.padding(16)
            .background(Color.dmCard, in: RoundedRectangle(cornerRadius: cornerRadius, style: .continuous))
    }

    func dmPill() -> some View {
        self.padding(.horizontal, 14).padding(.vertical, 8)
            .background(Color.dmCard, in: Capsule())
    }
...
struct FilledAccentButtonStyle: ButtonStyle {
    func makeBody(configuration: Configuration) -> some View {
        configuration.label
            ...
            .background(Color.dmAccent, in: Capsule())
